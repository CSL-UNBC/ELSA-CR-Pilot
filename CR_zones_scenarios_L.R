...
for(ii in 1:nlayers(zn1)){
  ...
  if(!names(zn1)[ii] == "Agriculture_Suitability"){
    ...
    for(jj in 1:100){
      ...
      fr <- feature_representation(p1_tmp,s1_tmp) 
      ...
      if(jj == 1){
        feat_rep_rel <- data.frame(rbind(c(fr$relative_held)))
        planning_u <- data.frame(rbind(c(pu)))
        names(feat_rep_rel) <- paste(fr$feature,"_",fr$zone)
      ...
      plot(category_layer(s1_tmp), main=(paste("Scenario",jj))) 
      print(paste("End of scenario",jj))
      rm(p1_tmp,s1_tmp, fr,pu)
    }
    print("Add common colun")
...
for(ii in 1:nlayers(zn1)){
  ...
  if(!names(zn1)[ii] == "Agriculture_Suitability"){
    ...
    for(jj in 1:100){
      ...
      plot(category_layer(s2_tmp), main=(paste("Scenario",jj))) 
      print(paste("End of scenario",jj))
      rm(p2_tmp,s2_tmp, fr,pu)
    }
    print("Add common colun")
    feat_rep_rel$scenario<- c(1:(nrow(feat_rep_rel)))
    planning_u$scenario<- c(1:(nrow(feat_rep_rel)))
...
for(jj in 1:100){
  p3_tmp <-   p3_glob %>% 
    add_max_utility_objective(c(count_tar(jj * 25 / (3 * 100)), 
                                count_tar(jj * 5 / (3 * 100)), 
                                count_tar(jj * 10 / (3 * 100)), 
                                100)) %>%
    add_feature_weights(w1)
  s3_tmp <- solve(p3_tmp, force=TRUE)
  ...
  writeRaster(category_layer(s3_tmp), 
              filename=here("output_trade_off", paste0("P25R05M10_Biod", sprintf("_%03d.tif", jj))), 
              overwrite=TRUE)
...
for(jj in 1:100){
  ...
  writeRaster(category_layer(s3_tmp), 
              filename=here("output_trade_off", paste0("P25R05M10_ES_no_carb", sprintf("_%03d.tif", jj))), 
              overwrite=TRUE)
...
for(jj in 1:100){
  ...
  writeRaster(category_layer(s3_tmp), 
              filename=here("output_trade_off", paste0("P25R05M10_ES", sprintf("_%03d.tif", jj))), 
              overwrite=TRUE)
...
for(jj in 1:100){
  ...
  writeRaster(category_layer(s3_tmp), 
              filename=here("output_trade_off", paste0("P25R05M10_CAR", sprintf("_%03d.tif", jj))), 
              overwrite=TRUE)
